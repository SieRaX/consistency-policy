...
from conditional_gradient_estimator.dataset.robomimic_replay_image_dataset_VAE import RobomimicReplayImageDatasetWrapper
...
class ConditionalGradientEstimationHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        dataset: RobomimicReplayImageDatasetWrapper
        root_dataset = hydra.utils.instantiate(cfg.task.dataset)
        dataset = hydra.utils.instantiate(cfg.wrapper_dataset, root_dataset=root_dataset, device=self.device)
        
        self.raw_data = {"data": list(), "condition": list()}
        for data in tqdm.tqdm(dataset, desc="Loading raw data", leave=False):
        ...
        assert isinstance(dataset, RobomimicReplayImageDatasetWrapper)
        ...
        B, D = data['data'].shape
        self.sampler = hydra.utils.instantiate(cfg.sampler, sample_dim=D)
