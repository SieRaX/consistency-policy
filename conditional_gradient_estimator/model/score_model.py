...
class DoubleScoreNet(nn.Module):
  def __init__(self, marginal_prob_std, data_dim=38, condition_dim=2, embed_dim=512, hidden_dim=512):
    super().__init__()
    self.score_net_uncondition = ScoreNet(marginal_prob_std, data_dim, condition_dim, embed_dim, hidden_dim)
    self.score_net_condition = ScoreNet(marginal_prob_std, data_dim, condition_dim, embed_dim, hidden_dim)
    
  def forward(self, x, t, condition=None):
    if condition is not None:
      return self.score_net_uncondition(x, t, condition)
    else:
      return self.score_net_condition(x, t, condition)
...
class DoubleScoreNet_Unet(nn.Module):
  def __init__(self, marginal_prob_std, condition_dim, input_channels=1, channels=[32, 64, 128, 256, 512, 1024], stride=[1, 2, 2, 2, 2, 2], padding=[1, 1, 1, 1, 1, 1], output_padding=[0, 0, 0, 0, 0, 0], kernel_size=3, embed_dim=256):
    super().__init__()
    self.score_net_uncondition = UnetBasedScoreNet(marginal_prob_std, condition_dim, input_channels, channels, stride, padding, output_padding, kernel_size, embed_dim)
    self.score_net_condition = UnetBasedScoreNet(marginal_prob_std, condition_dim, input_channels, channels, stride, padding, output_padding, kernel_size, embed_dim)
