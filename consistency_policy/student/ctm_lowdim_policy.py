...
class CTMPPUnetHybridLowdimPolicy(BaseLowdimPolicy):
    def __init__(self, 
            action_dim: int,
            obs_dim: int,
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_local_cond:
            ...
            trajectory = torch.cat([action, obs], dim=-1).detach()## 이거 detach??
        ...
        if "ctm" in self.losses.keys():
            ...
            noise_traj[condition_mask] = trajectory[condition_mask]
