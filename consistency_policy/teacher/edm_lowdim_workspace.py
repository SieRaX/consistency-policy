...
from consistency_policy.teacher.edm_lowdim_policy import KarrasUnetLowdimPolicy
...
class EDMLowdimWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: KarrasUnetLowdimPolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: KarrasUnetLowdimPolicy = None
    ...
    def run(self):
        ...
        if cfg.training.resume:
            if cfg.training.resume_path != "None":
                ...
                self.load_checkpoint(path=cfg.training.resume_path, exclude_keys=None)
                workspace_state_dict = torch.load(cfg.training.resume_path, weights_only=False)
            ...
            if lastest_ckpt_path.is_file() and cfg.training.resume_path == "None":
                ...
                workspace_state_dict = torch.load(lastest_ckpt_path, weights_only=False)
                normalizer = load_normalizer(workspace_state_dict)
                self.model.set_normalizer(normalizer)
        ...
        if not cfg.training.inference_mode:
            ...
            print(f"num_warmup_steps: {cfg.training.lr_warmup_steps}")
            print(f"num_training_steps: {len(train_dataloader) * cfg.training.num_epochs // cfg.training.gradient_accumulate_every}")
        ...
        # configure logging
        os.makedirs(str(self.output_dir), exist_ok=True)  # Create directory if it doesn't exist
...
def main(cfg):
    workspace = EDMLowdimWorkspace(cfg)
    workspace.run()
