...
class PushTKeypointsGradientRunner(BaseLowdimRunner):
    def __init__(self,
            output_dir,
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for i, vis, prefix in zip(np.arange(n_envs), env_vis, env_prefix):
            ...
            if len(image_frame) > 0:
                ...
                attention_data = np.array(spatial_attention_frame)
                ...
                graph_frames = []
                attention_bar = tqdm.tqdm(total=len(attention_data), 
                                        desc=f"Rendering Attention", 
                                        leave=False, 
