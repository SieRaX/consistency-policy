...
@click.option('-e', '--eval_uniform', default=False)
def main(path, attention_estimator_seed_dir, sub_dir="", device='cuda:0', c_init=1.0, d_init=1.0, max_steps=100, max_attention=15.0, eval_uniform=False):
    '''
    ...
    for seed_number in [9, 8, 7, 6, 5, 4, 3, 2, 1, 0]:
        ...
        if os.path.isdir(full_path):
            ...
            if eval_uniform:
                eval_uniform_dir="eval_uniform_by_length_ADP_code"
                false_true="true"
            else:
                eval_uniform_dir="eval_ADP_by_length"
                false_true="false"

            output_dir = os.path.join(full_path, eval_uniform_dir, '16_4')
            ...
            command = f"python eval_ADP_attention_estimator_by_length_cge_hydra_version.py \
                ...
                env_runner.min_n_action_steps=2 env_runner.attention_exponent=1.0 env_runner.n_test=100 env_runner.n_test_vis=0 env_runner.uniform_horizon={false_true} \
    ...
    json.dump(json_log, open(os.path.join(path, f'ADP_total_eval_statistics{postfix}.json'), 'w'), indent=2, sort_keys=True)
